...
class SystemMonitor:
    ...
    def _get_ip_addresses(self):
        """Get all IP addresses excluding those starting with 169"""
        ip_addresses = []
        try:
    ...
    def get_cpu_stats(self):
        ...
        try:
            ...
            current_freq = cpu_freq.current / 1000 if cpu_freq else 0  # Convert to GHz
            max_freq = cpu_freq.max / 1000 if cpu_freq else 5.0
        ...
        try:
            for partition in psutil.disk_partitions():
                ...
                try:
                    ...
                    disks[drive_letter.lower()] = {
                        'used': round(usage.used / (1024**3), 1),
                        'total': round(usage.total / (1024**3), 1),
                        'label': partition.device.split('\\')[-1] if '\\' in partition.device else 'Local Disk'
                    }
    ...
    <style>
        * {
        ...
        @media (max-width: 768px) {
            .system-container, .gpu-container, .disk-container, .network-container {
...
<body>
    ...
    <script>
        ...
        function getGreenRedGradient(percentage) {
            // CORRECTED: Green for low values (0%), red for high values (100%)
                    ...
                    panel.innerHTML = `
                        ...
                        <div class="stat-content">
                            <div class="drive-header">
                                <span class="drive-letter">${drive.toUpperCase()}:</span>
                                <span class="stat-value disk-usage-text" id="disk-${drive}-percent">${disk.label} ${percentage.toFixed(1)}%</span>
                            </div>
